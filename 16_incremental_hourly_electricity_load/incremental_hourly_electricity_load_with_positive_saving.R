...
for (province in provinces) {
  ...
  total_agg <- Reduce("+", lapply(tibble_list, `[[`, "aggregated_heating_cooling_with_positive_saving"))
  
  # Create a new tibble with the hour and total aggregated heating/cooling
  final_tibble <- tibble(hour = tibble_list[[1]]$hour, total_aggregated_heating_cooling_with_positive_saving = total_agg)
