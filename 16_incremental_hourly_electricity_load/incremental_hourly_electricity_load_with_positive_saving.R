...
for (province in provinces) {
  ...
  # Extract and sum the 'aggregated_heating_cooling_with_positive_saving' column
  total_agg_pos <- Reduce("+", lapply(tibble_list, `[[`, "aggregated_heating_cooling_with_positive_saving"))
  total_oldac_pos  <- Reduce(`+`, lapply(tibble_list, `[[`, "oldac_elec_use_hourly_with_positive_saving"))
  total_res_pos   <- Reduce(`+`, lapply(tibble_list, `[[`, "resistance_elec_use_hourly_with_positive_saving"))
  
  # Create a new tibble with the hour and total aggregated heating/cooling
  final_tibble <- tibble(
    hour = tibble_list[[1]]$hour,
    total_aggregated_heating_cooling_with_positive_saving = total_agg_pos,
    total_oldac_elec_use_hourly_with_positive_saving = total_oldac_pos,
    total_resistance_elec_use_hourly_with_positive_saving = total_res_pos
  )
  ...
  # Define file names
  file_base <- paste0(province_dir, "/total_incremental_hourly_electricity_load_with_positive_saving_", province, "_with_NG")
...
for (province in provinces) {
  ...
  # Define file names
  file_base <- paste0(province_dir, "/total_incremental_hourly_electricity_load_with_positive_saving_", province, "_no_NG")
