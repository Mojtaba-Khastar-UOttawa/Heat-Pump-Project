...
for (province in provinces) {
  ...
  rds_path <- sprintf("Results_paper/Results_Costmood/%s/total_incremental_hourly_electricity_load_with_positive_saving_%s_with_NG.rds", province, province)
  ...
  aggregated_data <- elec_load %>%
    ...
    mutate(aggregated_load = load + total_aggregated_heating_cooling_with_positive_saving
           - total_oldac_elec_use_hourly_with_positive_saving
           - total_resistance_elec_use_hourly_with_positive_saving)
  ...
  # Save the plot
  plot_path <- sprintf("Results_paper/Results_Costmood/%s/combined_elec_load_plot_%s_with_NG_highest_load_WPS.png", province, province)
  ggsave(plot_path, combined_elec_load_plot, width = 4.88, height = 3.88, units = "in", dpi = 300)
}

write_csv(percentage_increases_with_NG, "Results_paper/Results_Costmood/percentage_increases_with_NG_WPS.csv")
...
for (province in provinces) {
  ...
  rds_path <- sprintf("Results_paper/Results_Costmood/%s/total_incremental_hourly_electricity_load_with_positive_saving_%s_no_NG.rds", province, province)
  incremental_data <- readRDS(rds_path)
  ...
  # Save the plot
  plot_path <- sprintf("Results_paper/Results_Costmood/%s/combined_elec_load_plot_%s_no_NG_highest_load_WPS.png", province, province)
  ggsave(plot_path, combined_elec_load_plot, width = 4.88, height = 3.88, units = "in", dpi = 300)
}

write_csv(percentage_increases_no_NG, "Results_paper/Results_Costmood/percentage_increases_no_NG_WPS.csv")
