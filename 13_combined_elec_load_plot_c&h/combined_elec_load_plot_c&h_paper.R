...
aggregated_data <- aggregated_data %>%
  left_join(incremental_hourly_electricity_load %>% 
              select(hour, resistance_elec_use_hourly),
            by = c("hour_of_year" = "hour"))

# Create a new column that aggregate the electricity load and hp_elec_use
aggregated_data <- aggregated_data %>%
  mutate(aggregated_load = load + hp_elec_use_hourly - resistance_elec_use_hourly) %>%
  mutate(aggregated_load_c = load + hp_elec_use_c_hourly - oldac_elec_use_hourly) %>%
  mutate(aggregated_load_t = load + hp_elec_use_hourly + hp_elec_use_c_hourly - oldac_elec_use_hourly - resistance_elec_use_hourly)
...
combined_elec_load_plot_t <- ggplot() +
  ...
  labs(title = "Electricity Load and Aggregated Load with Heat Pump Electricity Usage for Heating and Cooling by Hour of Year",
       subtitle = "This Plot Presented for type of house Houses in Prov",
       x = "Hour of Year",
       y = "Load (MW)",
