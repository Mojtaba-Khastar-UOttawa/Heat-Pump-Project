...
if (furnace_type == "Natural Gas") {
    ...
    homes_data %>%
      ...
      hp_gas_use = 
        case_when(
          ...
          hc_high >= energy_demand & mood == "Cost" & hp_backup == "gas" & (ng_price+ng_emit*ctax/1000) / furnace_efficiency > ((elec_price / 100 / 0.0036) + (elec_emit*ctax/1000)) / hp_cop ~ 0,
          hc_high >= energy_demand & mood == "Cost" & hp_backup == "gas" & (ng_price+ng_emit*ctax/1000) / furnace_efficiency < ((elec_price / 100 / 0.0036) + (elec_emit*ctax/1000)) / hp_cop ~ (energy_demand / furnace_efficiency),
          hc_high >= energy_demand & hp_backup == "electricity" ~ 0,
          energy_demand > hc_high & hp_backup == "gas" ~ (energy_demand / furnace_efficiency),
          energy_demand > hc_high & hp_backup == "electricity" ~ 0
        ),
      furnace_gas_use = (energy_demand / furnace_efficiency)) %>%
      group_by(id,model) %>%
  ...
  for (f in unique_fsas) {
    ...
    homes_heating_fsa <- energy_calc_EFN(homes_data = homes_fsa, hourly_temperature = weather_fsa$temperature, hp_data = hpn, hp_backup = hp_backup, cp=cycling_cop_penalty, mood="Cost") 
  ...
  homes_heating <- homes_heating %>%
    ...
    inner_join(hp_equip_cost_calc(hp_equip_cost_data=hp_equip_cost_data,rebate=6250,ir=0.07,irr=0,life=15)) %>%
    
    mutate(
      
      annualized_furnace_cost = if_else(hp_backup == "electricity", 0, annualized_furnace_cost, missing = annualized_furnace_cost),
      ...
      hp_cost_saving = if_else(
        ...
        ac_cost + furnace_cost + annualized_furnace_cost + (12 * ng_monthly_fixed_cost) - hp_cost - annualized_hp_cost,
        ac_cost + furnace_cost + annualized_furnace_cost - hp_cost - annualized_hp_cost
      ),
  ...
  energy_calc_EFO <- function(homes_data, hourly_temperature, hp_data, cp=cycling_cop_penalty) {
    homes_data %>%
      ...
      furnace_oil_use = (energy_demand / furnace_efficiency)) %>%
      group_by(id,model) %>%
      summarise(hp_elec_use_h = sum(hp_elec_use_h, na.rm=T),
