...
if (furnace_type == "Natural Gas") {
  ...
  for (f in unique_fsas) {
    ...
    homes_heating_fsa <- energy_calc_EFN(homes_data = homes_fsa, hourly_temperature = weather_fsa$temperature, hp_data = hpn, hp_backup = "electricity", cp=cycling_cop_penalty, mood="Cost") 
