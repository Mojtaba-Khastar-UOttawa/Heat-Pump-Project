...
# Loop through each province and scenario
for (prov in provinces) {
  for (scen in scenarios) {
    filtered_data <- results %>%
      filter(province == prov, scenario == scen) %>%
      summarize(
...
write.csv(aggregated_results, "Results_paper/Results_Costmood/aggregated_summary_results_all_with_NG.csv", row.names = FALSE)
