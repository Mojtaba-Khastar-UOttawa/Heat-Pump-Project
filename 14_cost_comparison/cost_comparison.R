...
cost_comparison <- function(homes_data, hourly_temperature, hp_data, hp_equip_cost_data, rebate, ir, irr, ctax, life=15, hp_backup, cp=cycling_cop_penalty, mood) {
  ...
  if (furnace_type == "Natural Gas") {
    ...
    homes_heating <- homes_heating %>%
      left_join(homes_cooling[, c("id", "model", "hp_elec_use_c", "ac_elec_use")], by = c("id", "model"))
    homes_heating <- homes_heating %>%
    ...
    homes_heating <- homes_heating %>%
      ...
      mutate(
        annualized_furnace_cost = case_when(
          hp_backup == "electricity" ~ 0,
          TRUE ~ annualized_furnace_cost
        ),
        ...
        hp_cost_saving = case_when(
          hp_backup == "electricity" ~ ac_cost + furnace_cost + annualized_furnace_cost + (12 * ng_monthly_fixed_cost) - hp_cost - annualized_hp_cost,
          TRUE ~ ac_cost + furnace_cost + annualized_furnace_cost - hp_cost - annualized_hp_cost
        ),
    ...
    homes_heating <- homes_heating %>%
      ...
      mutate(
        ...
        hp_cost_saving = ac_cost + oil_furnace_cost - hp_cost - annualized_hp_cost,
    ...
    homes_heating <- homes_heating %>%
      ...
      mutate(
        ...
        ac_cost = ac_elec_use * ((elec_price / 100 / 0.0036) + (elec_emit*ctax/1000)),
        hp_cost = hp_elec_use * ((elec_price / 100 / 0.0036) + (elec_emit*ctax/1000)),
        hp_cost_saving = ac_cost + resistance_cost - hp_cost - annualized_hp_cost,
        hp_ghg = hp_elec_use * elec_emit / 1000,
...
mac_various <- 
  bind_rows(
    ...
    cost_comparison(homes_data=representative_homes, hourly_temperature = weather_fsa$temperature,hp_data=hpn, hp_equip_cost_data=hp_equip_cost_data, rebate=0, ir=0.07, irr=0., ctax=65, life=15, hp_backup=hp_backup, cp=cycling_cop_penalty, mood="Cost") %>%
      bind_cols(tibble(scenario = "ctax65")),
    cost_comparison(homes_data=representative_homes, hourly_temperature = weather_fsa$temperature,hp_data=hpn, hp_equip_cost_data=hp_equip_cost_data, rebate=0, ir=0.07, irr=0., ctax=170, life=15, hp_backup=hp_backup, cp=cycling_cop_penalty, mood="Cost") %>%
      bind_cols(tibble(scenario = "ctax170")),
    cost_comparison(homes_data=representative_homes, hourly_temperature = weather_fsa$temperature,hp_data=hpn, hp_equip_cost_data=hp_equip_cost_data, rebate=6250, ir=0.07, irr=0., ctax=170, life=15, hp_backup=hp_backup, cp=cycling_cop_penalty, mood="Cost") %>%
      bind_cols(tibble(scenario = "ctax170_+rebate")),
    cost_comparison(homes_data=representative_homes, hourly_temperature = weather_fsa$temperature,hp_data=hpn, hp_equip_cost_data=hp_equip_cost_data, rebate=6250, ir=0.07, irr=0.06, ctax=170, life=15,hp_backup=hp_backup, cp=cycling_cop_penalty, mood="Cost") %>%
      bind_cols(tibble(scenario = "ctax170_+rebate+lowinterest")),
    cost_comparison(homes_data=representative_homes, hourly_temperature = weather_fsa$temperature,hp_data=hpn, hp_equip_cost_data=hp_equip_cost_data, rebate=6250, ir=0.07, irr=0.06, ctax=170, life=15, hp_backup =hp_backup, cp=cycling_cop_penalty, mood="Cost") %>%
      bind_cols(tibble(scenario = "ctax170_+rebate+lowinterest+elecbackup"))
  )
