...
######## 1: Call EGH data and clean it (*** Province ***) and Select house type and heating system type (*** House type ***, *** Furnace type ***) ---- ("ON", "QC", "BC", "MB", "SK", "NS", "PE", "NF", "NB", "AB")

# Load the generalized data loading function
source("Scripts_Poster_1/Load_homes/load_EGH_homes_data.R")

# Define the list of provinces
province_codes <- c( "NS", "PE")
...
for (province in province_codes) {
  ...
  # Load data for the current province using the generalized function
  homes <- load_EGH_homes_data(province)
  
  # Check if data was loaded successfully
  if (is.null(homes) || nrow(homes) == 0) {
    # If no data, skip to the next province
    next
  }
...
source("Scripts_Poster_1/16_incremental_hourly_electricity_load/incremental_hourly_electricity_load.R")
source("Scripts_Poster_1/16_incremental_hourly_electricity_load/incremental_hourly_electricity_load_with_positive_saving.R")

source("Scripts_Poster_1/16_incremental_hourly_electricity_load/incremental_hourly_electricity_load_highest_load_plot.R")
source("Scripts_Poster_1/16_incremental_hourly_electricity_load/incremental_hourly_electricity_load_highest_load_plot_with_positive_saving.R")

###

source("Scripts_Poster_1/18_summary_results_all/summary_results_all_with_NG_paper.R")
source("Scripts_Poster_1/18_summary_results_all/summary_results_all_no_NG_paper.R")

source("Scripts_Poster_1/18_summary_results_all/aggregated_summary_results_all_with_NG_paper.R")
source("Scripts_Poster_1/18_summary_results_all/aggregated_summary_results_all_no_NG_paper.R")

source("Scripts_Poster_1/18_summary_results_all/country_aggregated_summary_results_all_with_NG_paper.R")
source("Scripts_Poster_1/18_summary_results_all/country_aggregated_summary_results_all_no_NG_paper.R")