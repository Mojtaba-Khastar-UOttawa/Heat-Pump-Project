...
if (furnace_type == "Natural Gas") {
  ...
  for (f in unique_fsas_hourly) {
    ...
    homes_heating_fsa_hourly <- energy_calc_hourly(homes_data_hourly = homes_fsa_hourly, hourly_temperature = weather_fsa_hourly$temperature, hp_data = hpn, hp_backup = "gas", cp=cycling_cop_penalty, mood="Cost") 
