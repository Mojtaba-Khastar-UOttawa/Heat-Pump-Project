...
optimal_plot_data_c <- homes_cooling_hourly %>%
  ...
  group_by(hour) %>%
  mutate(
    adjusted_elec_use_c_with_positive_saving = ifelse(saving >= 0, hp_elec_use_c_hourly * scale_factor_final, 0),
    adjusted_elec_use_c_with_positive_saving_oldac = ifelse(saving >= 0, oldac_elec_use_hourly * scale_factor_final, 0)) %>%
  summarise(
    ...
    oldac_elec_use_hourly_with_positive_saving = sum(adjusted_elec_use_c_with_positive_saving_oldac, na.rm = TRUE) * 0.2777777778,
    ac_elec_use_hourly = sum(ac_elec_use_hourly * scale_factor_final, na.rm = TRUE) * 0.2777777778,
    oldac_elec_use_hourly = sum(oldac_elec_use_hourly * scale_factor_final, na.rm = TRUE) * 0.2777777778
  )
