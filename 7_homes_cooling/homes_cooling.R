...
energy_calc_c <- function(homes_data, hourly_temperature, hp_data, cp=cycling_cop_penalty) {
  homes_data %>%
    ...
    left_join(hp_data) %>%
    # Define newac_cop: If ac_cop is not zero or NA, set newac_cop = 3.5, else 0
    mutate(newac_cop = ifelse(!is.na(ac_cop) & ac_cop != 0, 3.5, 0)) %>%
    ...
    ac_elec_use = energy_demand_c / newac_cop,
    oldac_elec_use = energy_demand_c / ac_cop) %>%
    group_by(id,model) %>%
    summarise(hp_elec_use_c = sum(hp_elec_use_c, na.rm=T),
              ac_elec_use = sum(ac_elec_use, na.rm=T),
              oldac_elec_use = sum(oldac_elec_use, na.rm = TRUE)) 
}
